...

{
	...
	// Data Edit
		...
		$sql2="SELECT `invo_no` FROM `adavnce_final_billing` WHERE date(`created_on`) BETWEEN '".$year."' AND '".$year_next."' AND `opd_flag`='".$opd_flag."' ORDER BY `id` DESC ";
		...
		$sql = "UPDATE  `adavnce_final_billing` SET `hospital_number`='".$hospital_number."',`patient_registration_id`='".$patient_registration_id."', `invo_no`='".$max_invo."', `name`='".$name."',`address`='".$address."',`mobile_prefix` ='".$mobile_prefix."',`mobile` ='".$mobile."',`email`='".$email."',`age` ='".$age."',`pt_type`='".$pt_type."',`discount`='".$discount."',`discount_purpose`='".$discount_purpose."',`discount_type`='".$discount_type."',`total`='".$total."',`doc_id`='".$doc_id."',`billing_date`=".$billing_date.",`billing_time`=".$billing_time.",`opd_flag`='".$opd_flag."',`admission_date`=".$admission_date." ,`admission_time`=".$admission_time." ,`date_of_discharge`=".$date_of_discharge." ,`time_of_discharge`=".$time_of_discharge." ,`gender`='".$gender."' ,`advance_payment_amt_text`='".$advance_payment_amt_text."',`instant_payment_amt_text`='".$instant_payment_amt_text."',`refund_payment_amt_text`='".$refund_payment_amt_text."',`net_payment_amt_text`='".$net_payment_amt_text."' ,`prefix`='".$prefix."',`modified_by`='".$one_time_edit_by."',`modified_time`='".$one_time_edit_time."'  WHERE `id`= '".$invoice_insert_id."' ";	
			...
			$id = $conn->insert_id;	
...

				   {
					   ...
					   if($_REQUEST['procedure_mode_flag'.$i]!='1')

			   			 {
							  ...
							  $sql4 = $conn->query("INSERT INTO `adavnce_final_procedure` SET `hospital_number`='".$hospital_number."',`patient_registration_id`='".$patient_registration_id."' ,`i_id` = '".$invoice_insert_id."',`procedure_id` = '".$_REQUEST['procedure_id'.$i]."',`amount` = '".$_REQUEST['amount'.$i]."',`discount` = '".$_REQUEST['discount'.$i]."',`net_amount` = '".$_REQUEST['net_amount'.$i]."',`surgery_flag` = '".$_REQUEST['surgery_flag'.$i]."',`procedure_mode_flag` = '1',`created_by`='".$one_time_edit_by."',`created_on`='".$one_time_edit_time."' ");					
						...
						if($_REQUEST['procedure_mode_flag'.$i]=='1')
							...
							$sql4 = $conn->query(" UPDATE `adavnce_final_procedure` SET `hospital_number`='".$hospital_number."',`patient_registration_id`='".$patient_registration_id."' ,`i_id` = '".$invoice_insert_id."',`procedure_id` = '".$_REQUEST['procedure_id'.$i]."',`amount` = '".$_REQUEST['amount'.$i]."',`discount` = '".$_REQUEST['discount'.$i]."',`net_amount` = '".$_REQUEST['net_amount'.$i]."',`surgery_flag` = '".$_REQUEST['surgery_flag'.$i]."',`procedure_mode_flag` = '1',`modified_by`='".$one_time_edit_by."',`modified_time`='".$one_time_edit_time."'  WHERE `id` = '".$_REQUEST['procedure_insert_id'.$i]."' ");						
				...
				if($remove_id_for_procedure!='')
			   {
				   ...
				   for($i=0;$i<($itemCount-1);$i++) {				  
					 ...
					 $sql4 = $conn->query("UPDATE `adavnce_final_procedure` SET `del_flag` = '1', `deleted_time`='".$one_time_edit_time."' , `deleted_by`='".$one_time_edit_by."' WHERE `id`='".$remove_id_array[$i]."' ");
		...
		$p_action="S";
...

				   {
						 ...
						 if($_REQUEST['payment_mode_flag'.$i]=='1')

			   			 {
							  ...
							  $sql4 = $conn->query("UPDATE `adavnce_final_payment_billing` SET `hospital_number`='".$hospital_number."',`patient_registration_id`='".$patient_registration_id."' ,`i_id` = '".$invoice_insert_id."',`p_key` = '".$_REQUEST['p_key'.$i]."',`p_value` = '".$_REQUEST['p_value'.$i]."',`p_action` = '".$p_action."',`tpa_name` = '".$_REQUEST['tpa_name'.$i]."',`claim_no` = '".$_REQUEST['claim_no'.$i]."',`payment_date` = '".date("Y-m-d", strtotime($_REQUEST['payment_date'.$i]))."',`payment_time` = '".date("H:i", strtotime($_REQUEST['payment_time'.$i]))."',`payment_type` = '".$_REQUEST['payment_type'.$i]."',`payment_mode_flag` = '1',`modified_by`='".$one_time_edit_by."',`modified_time`='".$one_time_edit_time."' WHERE `id` = '".$_REQUEST['payment_mode_insert_id'.$i]."'");	
...

		   {
			   ...
			   for($i=0;$i<($itemCount-1);$i++) {				  
			 	 ...
			 	 $sql4 = $conn->query("UPDATE `adavnce_final_payment_billing` SET `del_flag` = '1', `deleted_time`='".$one_time_edit_time."' , `deleted_by`='".$one_time_edit_by."' WHERE `id`='".$remove_id_array[$i]."' ");
			...
			$redirectUrlPrint=ADMIN_URL.'print_advance_final_bill.php?id='.$invoice_insert_id;
          ...
          <div class="portlet light">
            <div class="portlet-title">
              <div class="caption"> <i class="fa fa-cogs font-blue-sharp"></i> <span class="caption-subject font-blue-sharp bold uppercase">
                <?php if($row['opd_flag']=='1'){ echo 'OPD Advance ';}else{ echo 'IPD Advance ';} ?>
                Billing</span></div>
              ...
              <form action="" method="post" enctype="multipart/form-data" class="horizontal-form">
                <div class="form-body">
                  <div class="row" style="background:#dcefff;!important; padding:9px 0px">
                        ...
                        <div class="table-scrollable">
                          <table class="table table-striped table-bordered table-advance table-hover" id="iop_tabc">
                            ...
                            <?php
								 ...
								 $sql2="SELECT * FROM `adavnce_final_procedure` WHERE `i_id`= '" .$_REQUEST['id']. "' and `del_flag`='0'";
                              ...
                              <td ><textarea name="procedure_id<?php echo ($count+1); ?>" id="procedure_id<?php echo ($count+1); ?>"  class="form-control " placeholder="Enter Advance Purpose"></textarea><input type="hidden" name="procedure_mode_flag<?php echo $sl; ?>" id="procedure_mode_flag<?php echo $sl; ?>" value="0">
                              </td>
                              <td ><input type="phone"  autocomplete="off" class="form-control" placeholder="Enter Text" name="amount<?php echo ($count+1); ?>" id="amount<?php echo ($count+1); ?>" onKeyUp="value=value.replace(/[^\d]/g,'');amount('amount<?php echo ($count+1); ?>','<?php echo ($count+1); ?>');calculate_sales();" ></td>
                              <td ><input type="phone"  autocomplete="off" class="form-control" placeholder="Enter Text" name="discount<?php echo ($count+1); ?>" id="discount<?php echo ($count+1); ?>"  onKeyUp="value=value.replace(/[^\d]/g,'');discount_amount('discount<?php echo ($count+1); ?>','<?php echo ($count+1); ?>');calculate_sales();" ></td>
                              <td ><input type="phone"  autocomplete="off" class="form-control" placeholder="Enter Text" name="net_amount<?php echo ($count+1); ?>" id="net_amount<?php echo ($count+1); ?>" onKeyUp="value=value.replace(/[^\d]/g,'');calculate_sales();"  >
                            ...
                            <tr id="iopc<?php echo $sl; ?>">
                              ...
                              <textarea name="procedure_id<?php echo $sl; ?>" id="procedure_id<?php echo $sl; ?>"  class="form-control " placeholder="Enter Advance Purpose"><?php echo $row2['procedure_id'];?></textarea></td>
                    ...
                    <div class="col-md-12" style="text-align:left;font-weight:bold;color:blue;font-size:16px;"><b>Net Total: </b><span id="net_tot" style="color:red;"><?php echo $row['total'];?></span>
                    ...
                    <table class="table table-striped table-bordered table-advance table-hover" id="payment_mode_tab" style="overflow-x: auto !important;">
                      ...
                      <tbody>
                        <?php
								 ...
								 $sql2="SELECT * FROM `adavnce_final_payment_billing` WHERE `i_id`= '" .$_REQUEST['id']. "' and `del_flag`='0' ";
                        ...
                        <tr id="payment_mode_iop<?php echo $sl; ?>" style="background: #fff !important;">
                          <td ><?php echo $sl; ?></td>
                          <td ><select name="p_key<?php echo $sl; ?>" id="p_key<?php echo $sl; ?>" class="form-control" onChange="option_payment(<?php echo $sl; ?>);">
                              <option value=""> Choose..</option>
                              <?php
                          ...
                          <td ><input type="phone"  autocomplete="off" class="form-control" placeholder="Enter Payment Amt." name="p_value<?php echo $sl; ?>" id="p_value<?php echo $sl; ?>"   value="<?php echo $row2['p_value'];?>" onKeyUp="value=value.replace(/[^\d]/g,'');summation_payment(<?php echo $sl; ?>);" onBlur="over_under_paid_calculation();" readonly="readonly">
                          ...
                          <td ><div class="input-group">
                              <input type="text" class="form-control form-control-inline date-picker" placeholder="Select Payment Time" name="payment_date<?php echo $sl; ?>" id="payment_date<?php echo $sl; ?>" value="<?php if($row2['payment_date']!='') { echo date("d-m-Y", strtotime($row2['payment_date']));} else{ echo date("d-m-Y"); }  ?>" readonly="readonly" />
                              <span class="input-group-addon" style="padding:0 !important; width:50% !important">
                              <input type="text" class="form-control form-control-inline time-picker" placeholder="Select Payment Time" name="payment_time<?php echo $sl; ?>" id="payment_time<?php echo $sl; ?>" value="<?php  if($row2['payment_time']!='') { echo date("h:i A", strtotime($row2['payment_time']));} else{ echo date("h:i A"); }?>"  readonly="readonly" />
                              </span> </div></td>
                          <td ><input type="text" class="form-control" placeholder="Enter claim no. / Cheque No." name="claim_no<?php echo $sl; ?>" id="claim_no<?php echo $sl; ?>"  value="<?php echo $row2['claim_no'];?>" readonly="readonly"></td>
                          <td ><select name="tpa_name<?php echo $sl; ?>" id="tpa_name<?php echo $sl; ?>" class="form-control select2" readonly="readonly">
                              <option value=""> Choose..</option>
                            </select></td>
                          <td ><select name="payment_type<?php echo $sl; ?>" id="payment_type<?php echo $sl; ?>" onChange="summation_payment(<?php echo $sl; ?>);over_under_paid_calculation();"  class="form-control" readonly="readonly">
                              <?php
                                   ...
                                   $query1="SELECT `id`, `payment_type_name` FROM `payment_type_masters` WHERE `del_flag`='0' AND `advance_flag`='1' ORDER BY `id`" ;
                                        ...
                                        while($rowd=mysqli_fetch_array($rslt)){
                                            ...
                                            echo '<option value="'.$rowd['id'].'" '; if($row2['tpa_name']==$rowd['id']) echo 'selected'; echo '>'.$rowd['payment_type_name'].'</option>'; 
                          ...
                          <td><a href="javascript:void(0);" id="iop_remove<?php echo $sl; ?>" onClick="remove_payment_mode_update('<?php echo $sl; ?>','<?php echo $row2['id'];?>');" class="btn btn-sm default" title="OK"><i class="fa fa-times" style="font-size:16px;color:red"></i></a></td>
                                </tr>
                                <?php $sl++; }} ?>
