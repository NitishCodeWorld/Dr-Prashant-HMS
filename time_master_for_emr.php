...
if(isset($_REQUEST['submit'])){
	...
	if($_REQUEST['id']!=''){	
		...
		$sql = "UPDATE `time_master_for_emr` SET `name`='".$name."' WHERE `id`='".$id."'";
		if($conn->query($sql)===TRUE)
		{
		$msg="Record updated successfully";
		$flg=0;
		$redirectUrl=ADMIN_URL.'time_master_for_emr.php?msg='.$msg.'&flg='.$flg;
	...
	//Edit Data
		...
		$name=mysqli_real_escape_string($conn,$_REQUEST['medication_individual']);
		$sql = "INSERT INTO `time_master_for_emr` SET `name`='".$name."'";
		if($conn->query($sql)===TRUE)
		{
		$flg=0;
...
if(isset($_REQUEST['delete'])){
	$sql = "DELETE FROM `time_master_for_emr` WHERE `id`='".$_REQUEST['delete']."'";
...
<div class="page-container"> 
  ...
  <!-- END PAGE HEAD --> 
   			 <?php 
				 $total_count=0;
				$sql="SELECT COUNT(DISTINCT `id`) AS `total_count` FROM `time_master_for_emr`";
				$result=$conn->query($sql) ;				
          ...
          <div class="portlet light">
            ...
            <div class="portlet-body">
              ...
              <table class="table table-striped table-hover table-bordered" id="sample_editable_1">
                ...
                <tbody>
                  <?php 
				  $sql3="SELECT * FROM `time_master_for_emr` ORDER BY `id` DESC";
...
try{
		...
		$.ajax({
						type : "POST",
						url : "<?php echo ADMIN_URL; ?>ajax_for_emr/time_ajax_for_emr.php",
						dataType : "json", 
						data : "id="+id,
