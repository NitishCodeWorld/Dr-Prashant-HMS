...
<div class="page-container">
  <div class="page-content">
	...
	$purch_invoice=0 ;
	if($purch_invoice=="0") $sql="select pharma_invoice.*,customer_master.*,pharma_invoice.status as `status_` from pharma_invoice left join customer_master on customer_master.id=pharma_invoice.customer_id where pharma_invoice.id='".$inv_id."'";
	
	$res=mysqli_query($conn,$sql) or die(mysqli_error($conn));
	
	$sql_mode="select * from payment_mode where pharma_invoice_id='".$inv_id."' AND `medicine_return_flag`='0' ";
	$res_mode=mysqli_query($conn,$sql_mode) or die(mysqli_error($conn));
	$mode="";
	$cash_patient=0;
	while($row_mode=mysqli_fetch_assoc($res_mode)){
		
		if($row_mode['amount']>0) { $mode=$mode." ".$row_mode['mode'].": &#8377;".$row_mode['amount']; 
		$cash_patient=1;
		}
		...
		$row=mysqli_fetch_assoc($res);						
				...
				/*$sql_patient_admission_info="SELECT `date`,`time` FROM `patient_registration_form` WHERE `patient_id`= '".$row['patient_id']."' ";	
				$res_patient_admission_info=mysqli_query($conn,$sql_patient_admission_info) or die(mysqli_error($conn));
				$row_patient_admission_info=mysqli_fetch_assoc($res_patient_admission_info);
				$admission_date="";
				if($row_patient_admission_info['date']!=''){
					$admission_date=date("d/m/Y", strtotime($row_patient_admission_info['date']));
				}
				$admission_time="";
				if($row_patient_admission_info['time']!=''){
					$admission_time=date("h:i A", strtotime($row_patient_admission_info['time']));
				}*/
				
				/*$sql_patient_discharge_info="SELECT `date_of_discharge`,`time_of_discharge` FROM `dischargecertificate` WHERE `patient_id`= '".$row['patient_id']."' ";	
				$res_patient_discharge_info=mysqli_query($conn,$sql_patient_discharge_info) or die(mysqli_error($conn));
				$row_patient_discharge_info=mysqli_fetch_assoc($res_patient_discharge_info);
				$date_of_discharge="";
				if($row_patient_discharge_info['date_of_discharge']!=''){
					$date_of_discharge=date("d/m/Y", strtotime($row_patient_discharge_info['date_of_discharge']));
				}
				$time_of_discharge="";
				if($row_patient_discharge_info['time_of_discharge']!=''){
					$time_of_discharge=date("h:i A", strtotime($row_patient_discharge_info['time_of_discharge']));
				}*/
 ...
 ?>
          ...
          <div class="portlet light">
            <div class="portlet-title" style="text-align:center !important; margin-top:-9px; padding-bottom:9px; margin-bottom:0"> <span class="portlet-title" style="text-align:center !important; padding-bottom:9px"><span class="portlet-title" style="text-align:center !important; padding-bottom:9px"><strong style="font-size:12pt !important"><?php echo $row_hospital_info['hospital_name'];  ?></strong><br>
              ...
              <center><strong style="font-size:10pt !important;text-decoration:underline;"><?php if($row['status_'] == 4){ echo "PROFORMA INVOICE CUM ADVANCE RECEIPT";}else if($row['status_'] == 2){ echo "GST INVOICE"; }else { echo "GST INVOICE";}  ?></strong></center>
              </div>
                  ...
                  <table class="ott" style="width:100%" cellpadding="0" cellspacing="0" align="center">
                    ...
                    <tr>
                      <td colspan="2" style="line-height:14px !important"><strong>Party Details:</strong>
                        ...
                        <br>
                        <strong>Admit Date:</strong> <?php echo $admission_date.' '.$admission_time;?><br>
                        <strong>Discharge Date:</strong> <?php echo $date_of_discharge.' '.$time_of_discharge;?><br>
                        <strong>Ref. Doctors:</strong> <?php echo $row['doctor_name'];?></td>
                      <td colspan="2" style="line-height:14px !important;padding-left:130px;"><strong>Invoice No:</strong><?php echo $row['order_number'];  ?><br>
                        ...
                        <strong>Patient Mob:</strong> <?php echo $row['phone']; ?></td>
                    </tr>
                ...
                <div class="col-md-12">
                  <table class="ot" style="width:100%" cellpadding="0" cellspacing="0" align="center">
                    ...
                    <?php
									$mrp_total=0;
									 if($purch_invoice=="0"){ 
										$sql="select sd.*,sd.qty as `quanitity`,pm.*,`unit_master`.`unit_name` from pharma_invoice_details as sd inner join item_master as pm on sd.item_id=pm.id  LEFT JOIN `unit_master` ON `unit_master`.`id`=`pm`.`unit_id` where sd.sales_id='".$inv_id."'";
									}
                  ...
                  <table class="ot" style="width:100%" cellpadding="0" cellspacing="0" align="center">
                    ...
                    <tr >
                      ...
                      <td style="border-top:1px solid #333 !important; border-right:1px solid #333 !important; padding:0 3px 0 3px; text-align:right !important;border-bottom:1px solid #333 !important;text-align:center !important;"><?php echo number_format(($gst_percen/($sl-1)),2); ?>%</td>
                      <td style="border-top:1px solid #333 !important; border-right:1px solid #333 !important; padding:0 3px 0 3px; text-align:right !important;border-bottom:1px solid #333 !important;text-align:center !important;"><?php echo number_format(($gst_percen_total/($sl-1)),2); ?></td>
                      <td style="border-top:1px solid #333 !important; border-right:1px solid #333 !important; padding:0 3px 0 3px; text-align:right !important;border-bottom:1px solid #333 !important;"><?php echo number_format(($gst_percen/($sl-1)),2); ?>%</td>
                      <td style="border-top:1px solid #333 !important; border-right:1px solid #333 !important; padding:0 3px 0 3px; text-align:right !important;border-bottom:1px solid #333 !important;text-align:center !important;"><?php echo number_format(($gst_percen_total/($sl-1)),2); ?></td>
