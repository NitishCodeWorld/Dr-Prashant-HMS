        ...
        <div class="modal fade" id="ajax_2" role="large" aria-hidden="true">
          <div class="modal-dialog" style="width:500px">
            <div class="modal-content">
              <div class="modal-body" id="mb"> Date Range:
                <input type="text" name="datefilter" id="datefilter" value="" />
                &nbsp;&nbsp;&nbsp;
                <input type='color' id="event_color" class='basic' value='#f594d0' />
                <br/>
                <BR/>
...
$('#calendar').fullCalendar({
		...
		dayRender: function (date, cell) {
		 ...
		 dayClick: function(date) {
			...
			if($("#doctors").val()!=""){
				$('#ajax_2').modal();
			}else alert('Please select a doctor');
    ...
    // this event fires whenever the month is changed
		...
		eventClick: function(info) {
			...
			$.ajax({
				url: "get_json_data_for_appt.php?flag=16",
				type: 'POST',
				dataType : "json",
				data: {
					"id": info.id
		...
		contentHeight: "auto",	
		events: {
		url: 'get_json_data_for_appt.php?flag=14',
		method: 'POST',
		data:function() {
      	return {
        doctor: $("#doctors").val(),
	 ...
	 $.ajax({
			...
			success : function(data) {
				...
				if(data.flag=="1"){
					...
					//$(".fc-event-container").css("background-color", "" + data.backgroundcolor + "");   
					reload_calendar();
					$('#calendar').fullCalendar( 'refetchEvents' );	
