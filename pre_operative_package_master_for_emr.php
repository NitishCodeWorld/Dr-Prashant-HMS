...
if(isset($_REQUEST['submit']))
{
	...
	if($_REQUEST['id']!=''){	
		...
		$pre_operative_package=mysqli_real_escape_string($conn,$_REQUEST['pre_operative_package']);
		$id=mysqli_real_escape_string($conn,$_REQUEST['id']);
		$ct=mysqli_real_escape_string($conn,$_REQUEST['ct']);
		
		
		$sql = "UPDATE `pre_operative_package_master_for_emr` SET `pre_operative_package`='".$pre_operative_package."' WHERE `id`='".$id."'";
		if($conn->query($sql)===TRUE)
		{
			
		$sql3 = "DELETE FROM `pre_operative_subpackage_master_for_emr` WHERE `pre_operative_package_id`='".$id."'";
		$result3=$conn->query($sql3);
		
		for($i=0;$i<=$ct;$i++) {

		  if ($_REQUEST['pre_operative_subpackage'.$i]!='')

		   {
		
			   $sql4 = $conn->query("INSERT INTO `pre_operative_subpackage_master_for_emr` SET `pre_operative_package_id` = '".$id."',`pre_operative_subpackage` = '".$_REQUEST['pre_operative_subpackage'.$i]."'");
		...
		$flg=0;
		$redirectUrl=ADMIN_URL.'pre_operative_package_master_for_emr.php?msg='.$msg.'&flg='.$flg;
	...
	//Edit Data
		...
		$sql = "INSERT INTO `pre_operative_package_master_for_emr` SET `pre_operative_package`='".$pre_operative_package."' ";
...
{
	$sql = "DELETE FROM `pre_operative_package_master_for_emr` WHERE `id`='".$_REQUEST['delete']."'";
	$result=$conn->query($sql);
	if ($conn->query($sql) === TRUE)
	{
	$sql4 = $conn->query("DELETE FROM `pre_operative_subpackage_master_for_emr` WHERE `pre_operative_package_id`='".$_REQUEST['delete']."'");
	$flg=0;
...
<div class="page-container"> 
  ...
  <!-- END PAGE HEAD --> 
   			 <?php 
				 ...
				 //$today=date('Y-m-d');
				 $total_count=0;
				$sql="SELECT COUNT(DISTINCT `id`) AS `total_count` FROM `pre_operative_package_master_for_emr`";
				$result=$conn->query($sql) ;				
        ...
        <div class="col-md-12"> 
          ...
          <div class="portlet light">
            <div class="portlet-title">
              <div class="caption"> <i class="fa fa-cogs font-green-sharp"></i> <span class="caption-subject font-green-sharp bold uppercase">Pre-Operative Package Details </span></div>
            ...
            <div class="portlet-body">
              ...
              <table class="table table-striped table-hover table-bordered" id="sample_editable_1">
                ...
                <tbody>
                  <?php 
				  $sql3="SELECT `id`,`pre_operative_package` FROM `pre_operative_package_master_for_emr` ORDER BY `id` DESC";
				  $result3=$conn->query($sql3) ;
				  $id=1;
				 while ($row3=mysqli_fetch_array($result3,MYSQLI_ASSOC))
				 {
                   ...
                   <tr >
                    <td> <?php echo $id; ?></td>
                    <td> <?php echo $row3['pre_operative_package']; ?></td> 
                      <td>  <?php 
				  $sql4="SELECT `pre_operative_subpackage` FROM `pre_operative_subpackage_master_for_emr` WHERE `pre_operative_package_id`='".$row3['id']."' ";
				  $result4=$conn->query($sql4) ;
				  $j=1;
				 while ($row4=mysqli_fetch_array($result4,MYSQLI_ASSOC))
				 {
					 ?> 
                   <?php echo $j; ?>. <?php echo $row4['pre_operative_subpackage']; ?><br/>
                      <?php 
				  $j++; }?> 
                    </td>
                      ...
                      <div class="modal-content"> 
                        ...
                        <div class="modal-body">
                          <div class="portlet box blue-hoki">
                            <div class="portlet-title" >
                              <div class="caption"> <i class="fa fa-gift"></i>Add New Pre-Operative Package</div>
                            </div>
                                  ...
                                  <div class="form-group">
                                    ...
                                    <div class="col-md-7">
                                      <div class="input-group"><span class="input-group-addon"> <i class="fa fa-stethoscope"></i> </span>
                                        ...
                                        <textarea name="pre_operative_package" id="pre_operative_package" class="form-control" placeholder="Enter text" required></textarea>
      ...
      $(document).ready( function() {       
		setTimeout('$("#alert_msg").hide()',3000);
			...
			$("#add_new").click(function(){
				var empty='';
				$("#pre_operative_package").val(empty);
				$("#id").val(empty);	
					
				});
				$("#model_close").click(function(){
				 window.location.href="<?php echo ADMIN_URL; ?>pre_operative_package_master_for_emr.php";	
...
try{
		...
		$.ajax({
						type : "POST",
						url : "<?php echo ADMIN_URL; ?>ajax_for_emr/pre_operative_package_ajax_for_emr.php",
						dataType : "json", 
						data : "id="+id,
						success : function(data) {						
							...
							try{
								 ...
								 $("#id").val(data.id);	
								 $("#pre_operative_package").val(data.pre_operative_package);
...
try{
		...
		$.ajax({
						type : "POST",
						url : "<?php echo ADMIN_URL; ?>ajax_for_emr/pre_operative_subpackage_master_ajax_for_emr.php",
						dataType : "json", 
						data : "id="+id,
						success : function(data) {						
						//alert(data.flag);
