...
$title_span="IPD Service Wise Billing Details From ".date("d-M-Y", strtotime($_REQUEST['from_date']))." To ".date("d-M-Y", strtotime($_REQUEST['to_date']));
?>
<div class="notprnt">
  <div class="col-md-12">
    <div class="col-md-3">
...
<table width="100%" border="1" cellspacing="0" cellpadding="0" id="my_data_Table" >
  <tr>
    <td colspan="13" style="padding:6px 4px; font-size:12pt; text-align:left; font-family:Arial, Helvetica, sans-serif"><strong><?php echo $title_span; ?></strong></td>
  </tr>
  <?php 
			  ...
			  $due_amt=0;
			   ...
			   $doc_sl=1;
			   $sql_doc=" SELECT DISTINCT (`invoice_final_billing`.`doc_id`) AS `distinct_doc_id` FROM `invoice_final_billing` INNER JOIN `user_infos` ON `invoice_final_billing`.`doc_id`=`user_infos`.`users_id`  WHERE DATE(`invoice_final_billing`.`billing_date`) BETWEEN '".$from_date."' AND  '".$to_date."'  $extra_query AND `invoice_final_billing`.`del_flag`='0' ORDER BY `user_infos`.`name` ASC ";                   
	 ...
	 if($count_doc>'0'){
		 while ($row_doc=mysqli_fetch_array($result_doc,MYSQLI_ASSOC))
			 ...
			 echo '<tr><td colspan="13" style="padding:4px;"><center><span style="font-size:13pt;font-weight:bold !important;">'.$primary_doctor.'</span></center></td></tr>' ;
			 
			$doc_where="";
			 $doc_where=" AND `invoice_final_billing`.`doc_id`='".$row_doc['distinct_doc_id']."' ";	  
 
  $sql3=" SELECT `invoice_final_billing`.* FROM  `invoice_final_billing` WHERE DATE(`invoice_final_billing`.`billing_date`) BETWEEN '".$from_date."' AND  '".$to_date."'  $extra_query  AND  `invoice_final_billing`.`del_flag`='0' $doc_where ORDER BY `invoice_final_billing`.`billing_date` ASC "; 
   ...
   $result3=$conn->query($sql3) ;
	...
	echo '<tr ><td class="bold_td bord_bott_final" > </td><td class="bold_td bord_bott_final" > </td><td class="bold_td bord_bott_final" > </td><td class="bold_td bord_bott_final" > </td><td class="bold_td bord_bott_final" > </td><td class="bold_td bord_bott_final" > </td><td class="bold_td bord_bott_final_tot" >Total Of  '.$primary_doctor.' :</td><td class="bold_td bord_bott_final">'.$procedure_fecth_amount_net_tot.'</td><td class="bold_td bord_bott_final">'.$procedure_fecth_discount_net_tot.'</td><td class="bold_td bord_bott_final">'.$procedure_fecth_net_amount_net_tot.'</td><td class="bold_td bord_bott_final">'.$total_recieved_cash_net_tot.'</td><td class="bold_td bord_bott_final"> </td><td class="bold_td bord_bott_final">'.$due_amt_net_tot.'</td></tr>';
...
<script>
 $(document).ready( function() {  
	$('#exportBtn').on('click', function () {
		...
		$.ajax({
			url: 'ipd_export_service_wise_data.php?from_date='+from_date+'&to_date='+to_date,
			type: 'POST',
