...
function dr_pb_prescription_fetch_from_trenetralaya(){
	...
	$sql4="SELECT * FROM `prescription_details` WHERE `mrd_no`='".$mrd."' AND `id`='".$id."' ORDER BY `id` DESC LIMIT 1 ";
...
function fetch_old_patient_basic_data(){
			...
			$old_system_mrd=$_POST['old_system_mrd'];

			

			$sql="SELECT * FROM `prescription_details` WHERE `mrd_no`='".$old_system_mrd."' ORDER BY `id` DESC";

			$res=mysqli_query($conn2,$sql) or die(mysqli_error($conn2));
