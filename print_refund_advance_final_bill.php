...
$qry = mysqli_query($conn, "select * from `refund_adavnce_final_billing` where `id`='".$id."'"); // select query
...
if($del_flag=='1'){
	...
	$cancled=" Refund Advance ( Cancelled Bill )";

}else{
	$cancled=" Refund Advance Bill ";	
}
if($row3['opd_flag']==1){
	$bill_for='REF/OPD';
}else{
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td colspan="3" style="text-align:left; vertical-align:top;font-size:14px;"><strong>Received With Thanks Rupees <?php echo ucwords(number_to_indian_rupees_convert($row3['total']));?> Only. By <?php echo $pay_mode;  ?></strong></td>
  </tr>
  <tr>
    <td colspan="3">&nbsp;</td>
  </tr>
  <tr>
    <td colspan="3" style="border: 2px solid #333; padding:0px; text-align:center"><table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <?php
		 $sql_payment_details = "SELECT `refund_adavnce_final_payment_billing`.* , `payment_mode_masters`.`payment_mode_name`,`payment_type_masters`.`payment_type_name` FROM `refund_adavnce_final_payment_billing` INNER JOIN `payment_mode_masters` ON `refund_adavnce_final_payment_billing`.`p_key` =`payment_mode_masters`.`id` INNER JOIN `payment_type_masters` ON `refund_adavnce_final_payment_billing`.`payment_type` =`payment_type_masters`.`id`  WHERE `refund_adavnce_final_payment_billing`.`i_id` = '".$id."' AND `refund_adavnce_final_payment_billing`.`p_key`<>'5'  AND `refund_adavnce_final_payment_billing`.`del_flag`='0' ";
