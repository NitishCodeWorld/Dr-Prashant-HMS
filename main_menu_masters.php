...
if(isset($_REQUEST['submit']))
{
	// Edit Data 
	if($_REQUEST['id']!=''){	
		$main_menu_name=mysqli_real_escape_string($conn,$_REQUEST['main_menu_name']);
		...
		$right_to_left_navigation=mysqli_real_escape_string($conn,$_REQUEST['right_to_left_navigation']);
		
		$sql = "UPDATE `main_menu_masters` SET `main_menu_name`='".$main_menu_name."',`modified_by`='".$modified_by."' ,`modified_time`='".$modified_time."'  ,`right_to_left_navigation`='".$right_to_left_navigation."' WHERE `id`='".$id."'";
		if($conn->query($sql)===TRUE)
		{
		$msg="Record updated successfully";
		$flg=0;
		$redirectUrl=ADMIN_URL.'main_menu_masters.php?msg='.$msg.'&flg='.$flg;
	...
	//Add Data
		...
		$right_to_left_navigation=mysqli_real_escape_string($conn,$_REQUEST['right_to_left_navigation']);
		$sql = "INSERT INTO `main_menu_masters` SET `main_menu_name`='".$main_menu_name."',`created_by`='".$created_by."',`created_on`='".$created_on."' ,`right_to_left_navigation`='".$right_to_left_navigation."'";
		if($conn->query($sql)===TRUE)
		{
		$flg=0;
...
if(isset($_REQUEST['delete']))
{
	...
	$sql = "UPDATE `main_menu_masters` SET `del_flag`='1',`deleted_by`='".$deleted_by."',`deleted_time`='".$deleted_time."' WHERE `id`='".$_REQUEST['delete']."'";
...
<div class="page-container"> 
  ...
  <?php 
				 ...
				 //$today=date('Y-m-d');
				 $total_count=0;
				$sql="SELECT COUNT(DISTINCT `id`) AS `total_count` FROM `main_menu_masters` WHERE `del_flag`='0'";
				$result=$conn->query($sql) ;				
        ...
        <div class="col-md-12"> 
          ...
          <div class="portlet light">
            <div class="portlet-title">
              <div class="caption"> <i class="fa fa-cogs font-green-sharp"></i> <span class="caption-subject font-green-sharp bold uppercase">Main Menu Master</span></div>
            ...
            <table class="table table-striped table-hover table-bordered" id="sample_editable_1">
              ...
              <thead>
                <tr>
                  ...
                  <th>Right To Left Navigation</th>
                  <th>Details</th>
                  <th>Action</th>
                </tr>
              </thead>
              <tbody>
                <?php 
				  $sql3="SELECT `id`, `main_menu_name`,`created_by`,`created_on`,`modified_by`,`modified_time`,`del_flag`,`deleted_by`,`deleted_time` FROM `main_menu_masters` WHERE `del_flag`='0' ORDER BY `id` DESC";
				 ...
				 while ($row3=mysqli_fetch_array($result3,MYSQLI_ASSOC))
				 {
					 ...
					 $right_to_left_navigation='No';
					 if($row3['modified_by']==1){
						  $right_to_left_navigation='Yes';
					 }
					 ?>
                <tr >
                  <td><?php echo $id; ?></td>
                  <td><?php echo $row3['main_menu_name']; ?></td>
                  <td><?php echo $right_to_left_navigation; ?></td>
                  <td><b>Created By: </b><?php echo $created_by; ?> <br/>
                    ...
                    <?php } ?></td>
                  <td><a onClick="check('<?php echo $row3['id']; ?>')"  href="javascript:void(0);" ><i class="fa fa-edit" title="Edit"></i> </a> | <a onclick="if(confirm('Are you sure delete?')) return true; else return false;"  href="?delete=<?php echo $row3['id']; ?>" title="Delete / Cancel"><i class="fa fa-trash" title="Delete"></i> </a></td>
                </tr>
                <?php 
				  $id++; }?>
              </tbody>
                            ...
                            <div class="form-group">
                              ...
                              <div class="col-md-7">
                                <div class="input-group"><span class="input-group-addon"> <i class="fa fa-stethoscope"></i> </span>
                                  <input type="text" name="main_menu_name" id="main_menu_name" class="form-control" placeholder="Enter text" autocomplete="off" required/>
                              ...
                              <label class="col-md-4 control-label">Right To Left Navigation</label>
                              <div class="col-md-7">
                                <div class="input-group"><span class="input-group-addon"> <i class="fa fa-stethoscope"></i> </span>
                                  <select name="right_to_left_navigation" id="right_to_left_navigation" class="form-control">
      ...
      $(document).ready( function() {       
		setTimeout('$("#alert_msg").hide()',3000);
			...
			$("#add_new").click(function(){
				var empty='';
				$("#main_menu_name").val(empty);
				$("#id").val(empty);
				$("#right_to_left_navigation").val('0');					
				});
      });
function check(id){
...
try{
		...
		$.ajax({
						type : "POST",
						url : "<?php echo ADMIN_URL; ?>ajax/main_menu_ajax.php",
						dataType : "json", 
						data : "id="+id,
						success : function(data) {						
							...
							try{
								 ...
								 $("#main_menu_name").val(data.main_menu_name);
								 $("#id").val(data.id);	
								 $("#right_to_left_navigation").val(data.right_to_left_navigation);							
