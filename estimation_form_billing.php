...
if(isset($_REQUEST['register'])){
  ...
  //if($discharge_id==''){
	$sql="insert into estimation_form_details set  `name`='".$name."',  `address`='".$address."',  `mobile_prefix`='".$mobile_prefix."',  `mobile`='".$mobile."',  `age`='".$age."',  `doc_id`='".$doc_id."',  `pt_type`='".$pt_type."',  `billing_date`=".$billing_date.",  `system_created_date`=".$crdate.",  `billing_time`=".$billing_time.",  `total`='".$total."', `created_by`='".$created_by."',  `created_on`='".$created_on."', `gender`='".$gender."', `prefix`='".$prefix."'";
	  mysqli_query($conn,$sql) or die(json_encode(array("flag"=>"0","mysql_error"=>mysqli_error($conn)."Patient Insert Sucessfully")));
	  $est_id=mysqli_insert_id($conn);
	    /*Diagnoses Entry*/
                          ...
                          <table class="table table-bordered table-advance table-hover" id="iop_tabc">
                            ...
                            <tbody id="diagnoses_tbody">
                              <?php 
								...
								?>
                            <input type="hidden" name="countiopc" id="countiopc" value="1">
                            <tr id="iopc<?php echo $sl; ?>">
                              <td ><textarea name="procedure_id<?php echo $sl; ?>" id="procedure_id<?php echo $sl; ?>"  class="form-control " placeholder="Enter  Purpose"></textarea></td>
                              <td ><input type="phone"  autocomplete="off" class="form-control" placeholder="Enter Text" name="amount<?php echo $sl; ?>" id="amount<?php echo $sl; ?>" onKeyUp="value=value.replace(/[^\d]/g,'');amount('amount<?php echo $sl; ?>','<?php echo $sl; ?>');calculate_sales();" value="0"></td>
                              <td ><input type="phone"  autocomplete="off" class="form-control" placeholder="Enter Text" name="discount<?php echo $sl; ?>" id="discount<?php echo $sl; ?>"  onKeyUp="value=value.replace(/[^\d]/g,'');discount_amount('discount<?php echo $sl; ?>','<?php echo $sl; ?>');calculate_sales();" value="0" ></td>
                              <td ><input type="phone"  autocomplete="off" class="form-control" placeholder="Enter Text" name="net_amount<?php echo $sl; ?>" id="net_amount<?php echo $sl; ?>" onKeyUp="value=value.replace(/[^\d]/g,'');calculate_sales();" value="0" >
                                <input type="hidden" name="procedure_mode_flag<?php echo $sl; ?>" id="procedure_mode_flag<?php echo $sl; ?>" value="0"></td>
