                ...
                <table class="table table-striped table-hover table-bordered" id="asset_entry_data">
                  <thead>
                    <tr>
                      ...
                      <th width="15%" >BillSeriesCode</th>
                      <th width="13%" >Adjusted Amount</th>
                      <th width="15%" >Total Rec Amt</th>
                    </tr>
                  </thead>
...
if(tab_asset_entry!="") tab_asset_entry.destroy();
	...
	$.ajax({
            ...
            url: 'get_json_data_for_report_details.php?flag=4',
			dataType: 'json',
			data: data_details,
			type: 'POST',
			beforeSend: function(){
			...
			success: function (data) {
			 ...
			 var amount_grand_total_adusted=0;			 
			 var print_url="";
			  $.each(data, function(index, element) {				 
			 	...
			 	$('#assets_body').html($('#assets_body').html()+'<tr ><td class="defualt_td">'+element.sl_inner+'</td><td class="defualt_td"><a href="'+print_url+'" target="_blank" title="Bill Print" style="color:#0a5595 !important;">'+element.bill_no+'</a></td><td class="defualt_td">'+element.hospital_number+'</td><td class="defualt_td">'+element.prefix+' '+element.name+'</td><td class="defualt_td">'+element.receipt_date+'</td><td class="defualt_td">'+element.bill_for+'</td><td class="defualt_td">'+element.adjusted_value+'</td><td class="defualt_td">'+element.advance_amt+'</td></tr>');
				amount_grand_total=parseInt(amount_grand_total)+parseInt(element.advance_amt);
				amount_grand_total_adusted=parseInt(amount_grand_total_adusted)+parseInt(element.adjusted_value);
