<?php global $folder_name;
$rootDir = realpath($_SERVER["DOCUMENT_ROOT"]);
$fonts_loc=$rootDir."/".$folder_name."/captcha/cfonts/CONSOLA.TTF";


class Captcha {
  // (A) PRIME THE CAPTCHA - GENERATE RANDOM STRING IN SESSION
  function prime ($length=4) {
    //$char = "abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";
	$char = "abcdefghijkmnpqrstuvwxyz123456789";
    $max = strlen($char) - 1;
    $random = "";
    for ($i=0; $i<=$length; $i++) {
  ...
  function draw ($output=1, $width=300, $height=100, $fontsize=26, $font="") {
	  global $fonts_loc;
	  $font=$fonts_loc;
    ...
    $background = "captcha/captcha-back.jpg";
  ...
  function verify ($check) {
    // (C1) CAPTCHA NOT SET!
    if (!isset($_SESSION["captcha"])) { throw new Exception("CAPTCHA NOT Checked PRIMED"); }
