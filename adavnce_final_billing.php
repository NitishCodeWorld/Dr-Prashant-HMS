...

{
	...
	// Data Edit
		...
		$sql = "INSERT INTO `adavnce_final_billing` SET `hospital_number`='".$hospital_number."',`patient_registration_id`='".$patient_registration_id."', `invo_no`='".$max_invo."', `name`='".$name."',`address`='".$address."',`mobile_prefix` ='".$mobile_prefix."',`mobile` ='".$mobile."',`email`='".$email."',`age` ='".$age."',`pt_type`='".$pt_type."',`discount`='".$discount."',`discount_purpose`='".$discount_purpose."',`discount_type`='".$discount_type."',`total`='".$total."',`status`='".$status."',`print_status`='1',`doc_id`='".$doc_id."',`created_by`='".$created_by."',`created_on`='".$created_on."' ,`billing_date`=".$billing_date.",`billing_time`=".$billing_time.",`opd_flag`='".$opd_flag."',`admission_date`=".$admission_date." ,`admission_time`=".$admission_time." ,`date_of_discharge`=".$date_of_discharge." ,`time_of_discharge`=".$time_of_discharge." ,`gender`='".$gender."' ,`advance_payment_amt_text`='".$advance_payment_amt_text."',`instant_payment_amt_text`='".$instant_payment_amt_text."',`refund_payment_amt_text`='".$refund_payment_amt_text."',`net_payment_amt_text`='".$net_payment_amt_text."' ,`prefix`='".$prefix."' ";	
				...
				for($i=1;$i<=$countiopc;$i++) {
				 ...
				 if($_REQUEST['amount'.$i]!='')

				   {
					 ...
					 $sql4 = $conn->query("INSERT INTO `adavnce_final_procedure` SET `hospital_number`='".$hospital_number."',`patient_registration_id`='".$patient_registration_id."' ,`i_id` = '".$id."',`procedure_id` = '".$_REQUEST['procedure_id'.$i]."',`amount` = '".$_REQUEST['amount'.$i]."',`discount` = '".$_REQUEST['discount'.$i]."',`net_amount` = '".$_REQUEST['net_amount'.$i]."',`surgery_flag` = '".$_REQUEST['surgery_flag'.$i]."',`created_by`='".$created_by."',`created_on`='".$created_on."' ,`procedure_mode_flag` = '1' ");						
		...
		$p_action="S";
			...
			$redirectUrlPrint=ADMIN_URL.'print_advance_final_bill.php?id='.$id;
          ...
          <div class="portlet light">
            <div class="portlet-title">
              <div class="caption"> <i class="fa fa-cogs font-blue-sharp"></i> <span class="caption-subject font-blue-sharp bold uppercase">
                <?php if($_REQUEST['opd_flag']=='1'){ echo 'OPD Advance ';}else{ echo 'IPD Advance ';} ?>
                Billing</span></div>
                        ...
                        <div class="table-scrollable">
                          <table class="table table-striped table-bordered table-advance table-hover" id="iop_tabc">
                            ...
                            <tr id="iopc<?php echo $sl; ?>">
                              <td ><textarea name="procedure_id<?php echo $sl; ?>" id="procedure_id<?php echo $sl; ?>"  class="form-control " placeholder="Enter Advance Purpose"></textarea></td>
                      ...
                      <tbody>
                        <?php
								 ...
								 $sl=1;								 
								  ...
								  echo '<input type="hidden" name="count_payment_mode" id="count_payment_mode" value="'.$sl.'">';
                        ...
                        <tr id="payment_mode_iop<?php echo $sl; ?>">
                          ...
                          <td ><input type="phone"  autocomplete="off" class="form-control" placeholder="Enter Payment Amt." name="p_value<?php echo $sl; ?>" id="p_value<?php echo $sl; ?>"  value="0" onKeyUp="value=value.replace(/[^\d]/g,'');summation_payment(<?php echo $sl; ?>);" onBlur="over_under_paid_calculation();" readonly="readonly">
                            <input type="hidden" name="payment_mode_flag<?php echo $sl; ?>" id="payment_mode_flag<?php echo $sl; ?>" value="0"></td>
                          <td ><div class="input-group">
                              <input type="text" class="form-control form-control-inline date-picker" placeholder="Select Payment Time" name="payment_date<?php echo $sl; ?>" id="payment_date<?php echo $sl; ?>" value="<?php echo date("d-m-Y");  ?>" readonly="readonly" />
                              <span class="input-group-addon" style="padding:0 !important; width:50% !important">
                              <input type="text" class="form-control form-control-inline time-picker" placeholder="Select Payment Time" name="payment_time<?php echo $sl; ?>" id="payment_time<?php echo $sl; ?>" value="<?php  echo date("h:i A");  ?>"  readonly="readonly" />
                              </span> </div></td>
                          <td ><input type="text" class="form-control" placeholder="Enter claim no. / Cheque No." name="claim_no<?php echo $sl; ?>" id="claim_no<?php echo $sl; ?>"  value="" readonly="readonly"></td>
