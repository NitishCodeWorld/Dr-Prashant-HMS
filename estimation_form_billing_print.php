...
$id = $_GET['est_id']; // get id through query string
...
$qry = mysqli_query($conn, "select * from `estimation_form_details` where `id`='".$id."'"); // select query
...
if($del_flag=='1'){
	...
	$cancled=" Estimation Receipt ( Cancelled )";

}else{
	$cancled=" Estimation Receipt ";	
}
if($row3['opd_flag']==1){
	$bill_for='RECP/OPD';
}else{
...
<title>Estimation Form Print</title>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td colspan="3" style="border: 2px solid #333; padding:6px; text-align:center"><table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td width="15%" style="border-right: 1px solid #333; border-bottom: 1px solid #333;"><strong>Patient Name</strong></td>
          <td width="35%" style="border-bottom: 1px solid #333; padding-left:4px;border-right: 1px solid #333;"><?php echo $prefix.' '.$row3['name'];  ?></td>
        ...
        <tr>
          ...
          <td style="padding-left:4px; border-bottom: 1px solid #333;border-right: 1px solid #333;"><?php 
  ...
  <tr>
    <td colspan="3" style="border: 2px solid #333; padding:0px; text-align:center"><table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td width="17%" style="border-bottom: 1px solid #333; border-right: 1px solid #333; text-align:left;"><strong>Estimation Purpose</strong></td>
          <td width="17%" style="border-bottom: 1px solid #333;border-right: 1px solid #333;"><strong>Amount</strong></td>
          <td width="17%" style="border-right: 1px solid #333; border-bottom: 1px solid #333; text-align:left; padding-right:9px"><strong>Discount</strong></td>
          <td width="14%" style="border-right: 1px solid #333; border-bottom: 1px solid #333; text-align:left; padding-right:9px"><strong>Net Amount</strong></td>
        </tr>
        <?php
		 $sql_payment_details = "SELECT * FROM `estimation_purpose_details` WHERE `estimation_form_id`= '" .$row3['id']. "' and `del_flag`<>1";
		...
		?>
        <tr>
          <td style="border-right: 1px solid #333; text-align:left;"><?php echo $row2_payment_details['procedure_id'];  ?></td>
          <td style="border-right: 1px solid #333;"><?php echo number_format(($row2_payment_details['amount']),2);  ?></td>
          <td style="border-right: 1px solid #333; text-align:left; padding-right:9px"><?php echo number_format(($row2_payment_details['discount']),2);  ?></td>
          <td style="border-right: 1px solid #333; text-align:left; padding-right:9px"><?php echo number_format(($row2_payment_details['net_amount']),2);  ?></td>
        </tr>
        <?php 	}
}
		?>
        <tr>          
          <td style="border: 1px solid #333; text-align:right; padding-right:9px;font-weight:bold;font-size:14px;" colspan="3">Total Amount</td>
          <td style="border: 1px solid #333; text-align:left; padding-right:9px;font-weight:bold;font-size:14px;"><?php echo number_format(($row3['total']),2);  ?></td>
        </tr>
      </table></td>
  </tr>
</table>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    ...
    <td style="text-align:right; padding-right:9px;font-size: 11pt;"><strong>For: Pushpanjali Family Eye Care Center</strong></td>
  </tr>
  <tr>
    <td colspan="2">&nbsp;</td>
  </tr>
